...
float fLocRadius[] = {
    5.0, 5.5, 4.0, 9.0, 8.0, 9.0, 9.0, 7.0, 7.0, 8.0, 8.0, 5.5, 5.5, 7.5, 7.5, 6.0, 6.0, 6.0, 6.0,
};
...
float fLocRadius_Secondary[] = {
    0.0, 5.5, 4.0, 9.0, 8.0, 9.0, 9.0, 6.0, 6.0, 8.0, 8.0, 5.0, 5.0, 6.5, 6.5, 0.0, 0.0, 5.0, 5.0,
};
...
*/
static qboolean
LineSegmentToSphereIntersect(trace_t *pTrace, float fRadius, const vec3_t vCntr, const vec3_t vStart, const vec3_t vEnd)
{
    vec3_t vDiff, vDir;
    float  fDist;
    float  fLSqrd;
...
==================
*/
static qboolean CM_TraceDeepSimple(
    trace_t     *results,
    const vec3_t vStart,
    const vec3_t vEnd,
    int          iBrushMask,
    int          iEntContents,
    const vec3_t vEntMins,
    const vec3_t vEntMaxs,
    const vec3_t vEntAngles,
    vec3_t       vTransStart,
    vec3_t       vTransEnd
)
{
...
==================
*/
static qboolean CM_TraceDeepSimple2(
    trace_t             *results,
    const orientation_t *orPosition,
    vec3_t               vOffset,
    float                fRad,
    const char          *pszTagName,
    int                  iLocation,
    const vec3_t         vTransStart,
    const vec3_t         vTransEnd
)
{
    vec3_t vCntr;
...
==================
*/
static void CM_TraceDeepSuccess(
    trace_t *results, const vec3_t vStart, const vec3_t vEnd, int iEntNum, int iEntContents, int iLocation
)
{
...
void SV_TraceDeep(trace_t *results, const vec3_t vStart, const vec3_t vEnd, int iBrushMask, gentity_t *touch)
{
    ...
    for (iLocation = 0; iLocation < MAX_HITLOCATIONS; iLocation++) {
        ...
        if (CM_TraceDeepSimple2(results, &orPosition, vOffset, fRad, pszTagName, iLocation, vTransStart, vTransEnd)) {
...
)
{
    ...
    // check if something was not hit first
    if (!CM_TraceDeepSimple(
            results, vStart, vEnd, iBrushMask, iEntContents, vEntMins, vEntMaxs, vEntAngles, vTransStart, vTransEnd
        )) {
        return qfalse;
    }
