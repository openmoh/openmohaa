...
void Sys_Init( void ) {
	...
	if (g_wv.osversion.dwMajorVersion < 4)
		Sys_Error ("OpenMoHAA requires Windows version 4 or greater");
	if (g_wv.osversion.dwPlatformId == VER_PLATFORM_WIN32s)
		Sys_Error ("OpenMoHAA doesn't run on Win32s");
...
void* Sys_GetGameAPI(void* parms)
{
    ...
    game_library = LoadLibrary(fn);

    //First try in mod directories. basepath -> homepath -> cdpath
    if (!game_library) {
        if (homepath[0]) {
            Com_Printf("Sys_GetGameAPI(%s) failed: \"%d\"\n", fn, GetLastError());
...
void* Sys_GetCGameAPI(void* parms)
{
    ...
    cgame_library = LoadLibrary(fn);

    //First try in mod directories. basepath -> homepath -> cdpath
    if (!cgame_library) {
        if (homepath[0]) {
            Com_Printf("Sys_GetCGameAPI(%s) failed: \"%d\"\n", fn, GetLastError());
            fn = FS_BuildOSPath(homepath, gamedir, gamename);
            cgame_library = LoadLibrary(fn);
        }
    }
    ...
    GetCGameAPI = (void* (*)(void*))GetProcAddress(cgame_library, "GetCGameAPI");
