...

{
	...
	if (IsActive())
	{
		if (m_edit.m_cursor >= m_iPreStep)
		{
			...
			// m_iPreStep < iLastChar check added in OM to avoid passing negative size to Q_strncpyz
	...
	// i < EDITFIELD_BUFSIZE check added in OM for extra safety
	...
	if (iLastChar < EDITFIELD_BUFSIZE && m_edit.m_buffer[iLastChar]) // iLastChar index check added in OM
	{
...

{
	...
	case K_DEL: // DELETE key, backspace is handled in CharEvent
		...
		{
			memmove(
				&m_edit.m_buffer[m_edit.m_cursor],
				&m_edit.m_buffer[m_edit.m_cursor + 1],
				len - m_edit.m_cursor
			);
...

{
	...
	// is it out of bounds?
	if (len >= EDITFIELD_BUFSIZE) // changed in OM for extra safety - originally this was len == EDITFIELD_BUFSIZE
	{
		return;
	}
