...
const char *Script::GrabNextToken( qboolean crossline )
{
	...
	while( ( *script_p > TOKENSPACE ) && !AtComment() )
	{
		...
		{
			glbs.Error( ERR_DROP, "Token too large on line %i in file %s\n", line, filename.c_str() );
		}
...
const char *Script::GetString( qboolean crossline )
{
	...
	while( *script_p != '"' )
	{
		...
		if ( token_p == &token[ MAXTOKEN ] )
		{
			glbs.Error( ERR_DROP, "String too large on line %i in file %s\n", line, filename.c_str() );
		}
	}
...
void Script::LoadFile( const char *name )
{
	...
	length = glbs.FS_ReadFile( name, ( void ** )&tempbuf, true );
	...
	buffer = ( byte * )glbs.Malloc( length + 1 );
...
void Script::LoadFile( const char *name, int length, const char *buf )
{
	...
	// create our own space
	this->buffer = ( const char * )glbs.Malloc( length );
	this->length = length;
	// copy the file over to our space
