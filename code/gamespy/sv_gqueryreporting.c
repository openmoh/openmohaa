...
static int do_connect(SOCKET sock, char *addr, int port, struct sockaddr_in *master_saddr);
...
)
{
    ...
    current_rec->qr_custom_handler = NULL;
}
			...
			void *userdata)
{
	...
	return do_connect(hbsock, qr_hostname, MASTER_PORT, &(*qrec)->master_saddr);
...
#else
	ret = sendto(qrec->hbsock, buf, strlen(buf), 0, (const struct sockaddr *)&qrec->master_saddr, sizeof(struct sockaddr_in));
#endif
