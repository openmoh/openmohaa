...
void CrateObject::TellNeighborsToFall
	(
	void
	)
...
void CrateObject::TellNeighborsToJitter
	(
	Vector vJitterAdd
	)
...

{
	...
	mass = 20 * ( size[ 0 ] * 0.625f * size[ 1 ] * 0.625f * size[ 2 ] * 0.625f );
	...
	vMins[ 0 ] = mins[ 0 ] + origin[ 0 ] + 12.0f;
	vMins[ 1 ] = mins[ 1 ] + origin[ 1 ] + 12.0f;
	vMins[ 2 ] = mins[ 2 ] + origin[ 2 ] + 18.0f;

	vMaxs[ 0 ] = maxs[ 0 ] + origin[ 0 ] - 12.0f;
	vMaxs[ 1 ] = maxs[ 1 ] + origin[ 1 ] - 12.0f;
	vMaxs[ 2 ] = maxs[ 2 ] + origin[ 2 ] - 18.0f;

	for( pEnt = G_NextEntity( NULL ); pEnt != NULL; pEnt = pNext )
	{
		pNext = G_NextEntity( pEnt );

		if( pEnt->entnum > entnum && pEnt->IsSubclassOfCrateObject() )
		{
			if( vMins[ 0 ] <= absmax[ 0 ] &&
				vMins[ 1 ] <= absmax[ 1 ] && 
				vMins[ 2 ] <= absmax[ 2 ] &&
				...
				absmin[ 2 ] <= vMaxs[ 2 ] )
			{
				Com_Printf( "^~^~^ WARNING: func_crate entities %i and %i have overlapping volumes near (%g %g %g) to (%g %g %g)\n",
					entnum, pEnt->entnum, vMins[ 0 ], vMins[ 1 ], vMins[ 2 ], vMaxs[ 0 ], vMaxs[ 1 ], vMaxs[ 2 ] );
			}
		}
	}
}

void CrateObject::CrateDebrisType
	(
	Event *ev
	)
...

{
	...
	if( m_vJitterAngles[ 0 ] > m_fJitterScale * 1.25f ||
		-( m_fJitterScale * 1.25f ) > m_vJitterAngles[ 0 ] )
	{
