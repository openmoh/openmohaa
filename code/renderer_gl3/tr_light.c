...
byte R_CalcLightCubeSideBits(trRefLight_t * light, vec3_t worldBounds[2])
{
	...
	for(cubeSide = 0; cubeSide < 6; cubeSide++)
	{
		...
		// convert from our coordinate system (looking down X)
		// to OpenGL's coordinate system (looking down -Z)
		Matrix4x4Multiply(quakeToOpenGLMatrix, tmpMatrix, viewMatrix);
		...
		Matrix4x4Multiply(projectionMatrix, viewMatrix, viewProjectionMatrix);
...
void R_ComputeFinalAttenuation(shaderStage_t * pStage, trRefLight_t * light)
{
	...
	Matrix4x4Multiply(matrix, light->attenuationMatrix, light->attenuationMatrix2);
}
