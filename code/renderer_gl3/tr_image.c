...
static void R_MipMap2(unsigned *in, int inWidth, int inHeight)
{
	...
	temp = ri.Malloc(outWidth * outHeight * 4);
...
void R_UploadImage(const byte ** dataArray, int numData, image_t * image)
{
	...
	scaledBuffer = ri.Malloc(sizeof(byte) * scaledWidth * scaledHeight * 4);
...
static void R_CreateShadowCubeFBOImage(void)
{
	...
	for(j = 0; j < 5; j++)
	{
		...
		for(i = 0; i < 6; i++)
		{
			data[i] = ri.Malloc(width * height * 4);
		}
