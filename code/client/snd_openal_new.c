...
*/
static
void S_AL_UpdateEntity( int entityNum, const vec3_t origin, const vec3_t velocity )
{
	vec3_t sanOrigin;
	...
	if ( entityNum < 0 || entityNum >= MAX_GENTITIES )
		Com_Error( ERR_DROP, "S_AL_UpdateEntity: bad entitynum %i", entityNum );
	VectorCopy( sanOrigin, entityList[ entityNum ].origin );
	VectorCopy( velocity, entityList[ entityNum ].velocity );
}
...
*/
static
qboolean S_AL_IsSoundPlaying( int channelNumber, const char *name )
{
	int i;

	for( i = 0; i < MAX_SRC; i++ )
	{
		if( srcList[ i ].channel == channelNumber && !strcmp( knownSfx[ srcList[ i ].sfx ].filename, name ) ) {
			if( srcList[ i ].isActive ) {
				return qtrue;
			} else {
				return qfalse;
...
qboolean S_AL_Init( soundInterface_t *si )
{
	...
	si->UpdateEntity = S_AL_UpdateEntity;
