...
static void UI_SetDMConsoleMode( int iMode ) {
	...
	if( bQuickMessage )
	{
		...
		{
			dm_console->setTitle( va( "%s %i", Sys_LV_CL_ConvertString( "Quick Chat (Press Enter to send) : Messaging to Client" ), iMode ) );
		}
	}
	else
	{
		if( iMode == 100 )
		{
			dm_console->setTitle( Sys_LV_CL_ConvertString( "Chat Window (Enter to send/close) (all|team|private to change mode) : Messaging to All" ) );
		}
		else if( iMode == 100 )
		{
			dm_console->setTitle( Sys_LV_CL_ConvertString( "Chat Window (Enter to send/close) (all|team|private to change mode) : Messaging to Team" ) );
		}
		else
		{
			dm_console->setTitle( va( "%s %i", Sys_LV_CL_ConvertString( "Chat Window (Enter to send/close) (all|team|private to change mode) : Messaging to Client" ), iMode ) );
		}
	}
}
...
void UI_StartDMMap_f( void ) {
	...
	Cbuf_AddText(
		...
		"wait;"
		"map \"%s\"\n",
		iDedicated, iMaxClients, iUseGameSpy, iGameType, pszGameTypeString,
		iFragLimit, iTimeLimit, iTeamDamage,
		iInactiveSpectate, iInactiveKick,
		pszMapListString, szHostName, pszMapName ) );
}
...
void UI_ServerLoaded( void ) {
	...
	if( pCurrentMenu != ui_pLoadingMenu
		|| !pCurrentMenu->GetNamedWidget( "continuebutton" )
...
void ScoreboardListItem::SetListItemStrings( const char *string1, const char *string2, const char *string3, const char *string4, const char *string5, const char *string6, const char *string7, const char *string8 )
{
	strings[ 0 ] = string1;
	strings[ 1 ] = string2;
	strings[ 2 ] = string3;
