...
void S_BeginRegistration( void ) {
	...
	if (s_numSfx == 0) {
		...
		S_RegisterSound("sound/feedback/hit.wav", qfalse, qfalse);		// changed to a sound in baseq3
	}
}
...
==================
*/
sfxHandle_t	S_RegisterSound( const char *name, qboolean compressed, qboolean streamed ) {
	sfx_t	*sfx;
...
void S_StartSound(const vec3_t origin, int entNum, int entChannel, sfxHandle_t sfxHandle, float volume, float minDist, float pitch, float maxDist, int streamed) {
  ...
  int	inplay, allowed;
	...
	if ( !origin && ( entNum < 0 || entNum > MAX_GENTITIES ) ) {
		Com_Error( ERR_DROP, "S_StartSound: bad entNum %i", entNum );
	}
...
//	Com_Printf("playing %s\n", sfx->soundName);
	...
	for ( i = 0; i < MAX_CHANNELS ; i++, ch++ ) {		
		if (ch[i].entnum == entNum && ch[i].thesfx == sfx) {
...
//				}
	...
	ch = S_ChannelMalloc();	// entNum, entChannel);
	if (!ch) {
		ch = s_channels;
...
void S_StartLocalSound( sfxHandle_t sfxHandle, int channelNum ) {
	...
	S_StartSound (NULL, listener_number, channelNum, sfxHandle, 1.f, -1, 1.f, -1, qfalse);
}
...
void S_Play_f( void ) {
	...
	while ( i<Cmd_Argc() ) {
		...
		h = S_RegisterSound( name, qfalse, qfalse );
		if( h ) {
...
void S_StartBackgroundTrack( const char *intro, const char *loop ){
	...
	//
	FS_FOpenFileRead( name, &s_backgroundFile, qtrue, qtrue );
	if ( !s_backgroundFile ) {
