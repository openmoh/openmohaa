...
void Q3_BSPBrushToMapBrush(dbrush_t *bspbrush, entity_t *mapent)
{
	...
	for (n = 0; n < bspbrush->numSides; n++)
	{
		...
		if (dbrushsidetextured[bspbrush->firstSide + n]) side->flags |= SFL_TEXTURED|SFL_VISIBLE;
		...
		{
			side->contents = dshaders[bspbrushside->shaderNum].contentFlags;
			side->surf = dshaders[bspbrushside->shaderNum].surfaceFlags;
			if (strstr(dshaders[bspbrushside->shaderNum].shader, "common/hint"))
			{
...
//		side_brushtextures[nummapbrushsides] = td;
	...
	b->contents = dshaders[bspbrush->shaderNum].contentFlags;
	b->contents &= ~(
#ifdef CONTENTS_LADDER
		CONTENTS_LADDER|
#endif
		CONTENTS_FOG|CONTENTS_STRUCTURAL);
...
void Q3_ParseBSPBrushes(entity_t *mapent)
{
	...
	for (i = 0; i < dmodels[mapent->modelnum].numBrushes; i++)
	{
		Q3_BSPBrushToMapBrush(&dbrushes[dmodels[mapent->modelnum].firstBrush + i], mapent);
	} //end for
...
void AAS_CreateCurveBrushes(void)
{
	...
	for (i = 0; i < q3_numDrawSurfaces; i++)
	{
		...
		if (!(dshaders[surface->shaderNum].contentFlags & (CONTENTS_SOLID|CONTENTS_PLAYERCLIP)))
		{
			//Log_Print("skipped non-solid curve\n");
			continue;
		} //end if
		// if this curve should not be used for AAS
		if ( dshaders[surface->shaderNum].contentFlags & CONTENTS_NOBOTCLIP ) {
			continue;
		}
		//
		...
		dv_p = drawVerts + surface->firstVert;
		for ( j = 0 ; j < c ; j++, dv_p++ )
		{
